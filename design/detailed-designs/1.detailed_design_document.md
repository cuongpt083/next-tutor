...
#### 2.4.5. Chat Completion (LLM Proxy)

* **Endpoint**: `POST /api/v1/ai/chat/completions`
* **Mô tả**: Giao tiếp trực tiếp với các mô hình LLM (OpenAI, Gemini, Grok) thông qua Adapter. Hỗ trợ stream response.
* **Request**:
    ...
    ```json
    {
      ...
      "model": "gpt-4o", // Optional, default configured in backend
      "temperature": 0.7,
      "stream": false
    }
    ```
...
#### 2.4.6. Text Summarization

* **Endpoint**: `POST /api/v1/ai/summarize`
* **Mô tả**: Tóm tắt nội dung văn bản hoặc hội thoại, phục vụ cho việc quản lý context và ôn tập.
* **Request**:

    ```json
    {
      "text": "Long text content...",
      "maxLength": 100 // Optional: max words/tokens
    }
    ```
...
#### 2.4.7. Grammar Analysis

* **Endpoint**: `POST /api/v1/ai/grammar/check`
* **Mô tả**: Phân tích ngữ pháp, chính tả và đề xuất sửa lỗi cho văn bản đầu vào.
* **Request**:
...
* **Response (200 OK)**:
    ...
    ```json
    {
      ...
      "improvedText": "I went to school yesterday."
    }
    ```

#### 2.4.8. Các Utility Function
Các Utility Function bao gồm các function hỗ trợ cho các API trên, được implement trong package `com.cuongpt.nexttutor.utils`. Các Utility Function bao gồm:

* `com.cuongpt.nexttutor.utils.APIKeyRotationUtils`: Cung cấp các function hỗ trợ cho việc lấy API Key ngẫu nhiên từ bảng `api_keys` và rotation API Key khi API Key hết hạn hoặc hết lượt gọi. Các function chính bao gồm:
    * `getRandomApiKey()`: Lấy API Key ngẫu nhiên từ bảng `api_keys`.
    * `rotateApiKey()`: Rotation API Key khi API Key hết hạn hoặc hết lượt gọi. Trả về API Key mới.
* `com.cuongpt.nexttutor.utils.SystemPromptUtils`: Cung cấp các function hỗ trợ cho việc tạo System Prompt bao gồm Master Prompt và System Prompt cho các Message Prompt. Các function chính bao gồm:
    * `createSystemPrompt()`: Tạo System Prompt bao gồm Master Prompt và System Prompt cho các Message Prompt.
* `com.cuongpt.nexttutor.utils.TokenUtils`: Cung cấp các function hỗ trợ cho việc tính toán token. Các function chính bao gồm:
    * `countTokens()`: Tính toán số lượng token trong một chuỗi.
* `com.cuongpt.nexttutor.utils.SummaryUtils`: Cung cấp các function hỗ trợ cho việc tóm tắt nội dung văn bản hoặc hội thoại, phục vụ cho việc quản lý context và ôn tập. Các function chính bao gồm:
    * `summarizeText()`: Tóm tắt nội dung văn bản hoặc hội thoại.
* `com.cuongpt.nexttutor.utils.ContextManager`: Cung cấp các function hỗ trợ cho việc quản lý context, tạo và gán các Message Prompt cho các yêu cầu tới AI Service Adapter. Các function chính bao gồm:
    * `createContext()`: Tạo context cho yêu cầu tới AI Service Adapter.
    * `assignMessagePrompt()`: Gán các Message Prompt cho yêu cầu tới AI Service Adapter.
* `com.cuongpt.nexttutor.utils.ProgressManager`: Cung cấp các function hỗ trợ cho việc quản lý tiến trình học tập của người dùng, cập nhật thông tin vào bảng `user_progress`. Các function chính bao gồm:
    * `updateUserProgress()`: Cập nhật thông tin tiến trình học tập của người dùng.
    * `getUserProgress()`: Lấy thông tin tiến trình học tập của người dùng.
...
-- Chat Completion Logs Table
CREATE TABLE chat_completion_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id UUID REFERENCES learning_sessions(id),
...
-- Grammar Check Logs Table
CREATE TABLE grammar_check_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id),
...
Thiết kế chi tiết về UI/UX cũng như hoạt động của component này trong file `detailed_design_ui_ux_modules_learning.md`.
...
Thiết kế chi tiết về UI/UX cũng như hoạt động của component này trong file `detailed_design_ui_ux_modules_common.md`.

#### 4.2.4. FeedbackModalComponent

* **Mục đích**: Hiển thị các gợi ý và thông tin về ngữ pháp.
* **Input**: `feedback: Feedback`.
* **UI/UX**:
  * Hiển thị các gợi ý và thông tin về ngữ pháp.
Thiết kế chi tiết về UI/UX cũng như hoạt động của component này trong file `detailed_design_ui_ux_modules_learning.md`.

#### 4.2.5. TutorSelectionComponent

* **Mục đích**: Chọn Tutor cho buổi học.
* **Output**: `onTutorSelected: EventEmitter<Tutor>`.
* **UI/UX**:
  * Hiển thị danh sách Tutor và thông tin về họ.
Thiết kế chi tiết về UI/UX cũng như hoạt động của component này trong file `detailed_design_ui_ux_modules_common.md`.

#### 4.2.6. LoginComponent

* **Mục đích**: Hiển thị form đăng nhập.
* **Output**: `onLogin: EventEmitter<User>`.
* **UI/UX**:
  * Hiển thị form đăng nhập.
Thiết kế chi tiết về UI/UX cũng như hoạt động của component này trong file `detailed_design_ui_ux_modules_auth.md`.

#### 4.2.7. RegisterComponent
...
#### 4.2.8. ForgotPasswordComponent

* **Mục đích**: Hiển thị form quên mật khẩu.
* **Output**: `onForgotPassword: EventEmitter<User>`.
* **UI/UX**:
  * Hiển thị form quên mật khẩu.
Thiết kế chi tiết về UI/UX cũng như hoạt động của component này trong file `detailed_design_ui_ux_modules_auth.md`.

#### 4.2.9. ResetPasswordComponent

* **Mục đích**: Hiển thị form reset mật khẩu.
* **Output**: `onResetPassword: EventEmitter<User>`.
* **UI/UX**:
